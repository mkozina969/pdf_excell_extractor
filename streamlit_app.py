...
st.set_page_config(page_title="Invoice PDF -> Excel (PLUS v3.3.20 stable)", layout="wide")
...
def to_xlsx_bytes(header, items) -> bytes:
    ...
    buf.seek(0); from openpyxl import load_workbook
...
def choose_best_bosch_conti(b: bytes, vendor_key: str):
    if vendor_key not in ("bosch_golden", "conti_golden"): return None
...
vendor_override = st.sidebar.selectbox("Vendor override", ["auto","diesel_golden","ngk_golden","bosch_golden","conti_golden"], index=0)
extractor = st.sidebar.selectbox("Extractor", ["auto","pdfplumber","pypdf","pymupdf"], index=0)
show_debug = st.sidebar.checkbox("Show debug (raw text + parsed items)", value=False)
...
if files:
    ...
    for f in files:
        ...
        st.write("ERP_Import"); df = pd.DataFrame(items); st.dataframe(df)
        ...
        if show_debug:
            st.write("Debug (first 200 lines)"); st.text("\\n".join((text or "").splitlines()[:200]))
    zf.close()

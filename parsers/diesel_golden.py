...
def parse(full_text: str) -> Tuple[Dict, List[Dict]]:
    """
    Parse Diesel Technic invoice PDF text content.
    ...
    for ln in [ln.strip() for ln in full_text.splitlines() if ln.strip()]:
        if re.search(
            r"ART\.?\s*NO\.?|DESCRIPTION|QTY\.|UNIT\b|PRICE\b|AMOUNT\b|CARRIED OVER",
            ln,
            re.I
        ):
            continue
        ...
        if (price_i-3 >= 0 and INT_RE.match(tokens[price_i-1]) and 
            UOM_RE.match(tokens[price_i-2]) and EU_RE.match(tokens[price_i-3])):
            qty_s = tokens[price_i-3]
            uom = tokens[price_i-2]
        elif (price_i-2 >= 0 and UOM_RE.match(tokens[price_i-1]) and 
              EU_RE.match(tokens[price_i-2])):
            qty_s = tokens[price_i-2]
            uom = tokens[price_i-1]
        else:
            continue
